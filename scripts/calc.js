(function() {
  ...
  let calculator = {
    ...
    _isExprEndsWithOperator() {
      return this.operators.indexOf(this.expr[this.expr.length - 1]) > -1 && !this.expr.endsWith(' -');
    },
    ...
    onClickEqual(event) {
      this._isExprEndsWithOperator() ?
        this.set(eval(this.expr.slice(0, -1))) :
        this.set(eval(this.expr));
    },

    onClickSquare(event) {
      this._isExprEndsWithOperator() ?
        this.set(Math.pow(eval(this.expr.slice(0, -1)), 2)) :
        this.set(Math.pow(eval(this.expr), 2));
    },
